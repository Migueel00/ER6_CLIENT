...
const IllnessScreen = () => {
  ...
  const [shownCurses, setShownCurses] = useState(require('../../../fakedata/fake-curses.json'));
  ...
  useEffect(() => {
    ...
    const formattedCurses = Array.isArray(shownCurses.data) ? shownCurses.data : [];

    console.log('FORMATED CURSES', formattedCurses);

    setShownCurses([{ key: 'left-spacer' }, ...formattedCurses, { key: 'right-spacer' }]);
  ...
  return (
    <Container>
      ...
      <ImageBackground source={backgroundImageURL} style={styles.backgroundImage}>
        ...
        <FlatListCurses curses={shownCurses} handleLongPress={handleLongPress} showNotFoundText={showNotFoundText} />
